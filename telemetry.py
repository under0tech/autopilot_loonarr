...
def telemetry_requestor(stop_command):
    while autopilot.state['connection'] == False and not stop_command.is_set():
        try:
            time.sleep(5)
            messages.display(messages.telemetry_process_connecting, 
                             [vars.mavlink_url])
    ...
    if not stop_command.is_set():
        messages.display(messages.telemetry_process_connected, 
                         [vars.mavlink_url])

    while not stop_command.is_set():
        try:            
            router.put_command(
                router.Command(3,'MONITOR',{'target':'SYS_STATUS'}))
            router.put_command(
                router.Command(2,'TELEMETRY',{'target':'RC_CHANNELS'}))
            # router.put_command(
            #     router.Command(1,'TELEMETRY',{'target':'LOCAL_POSITION_NED'}))
            time.sleep(4)
        except:
            pass
